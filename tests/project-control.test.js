...
test('dynamic invoice URLs are encoded before DOM insertion', () => {
  for (const [relative, expression] of [
    ['dashboard.html', 'encodeURIComponent(x.id)'],
    ['invoices.html', 'encodeURIComponent(x.id)'],
    ['invoice-preview.html', 'encodeURIComponent(id)'],
    ['new-invoice.html', 'encodeURIComponent(result.id)']
  ]) {
    assert.ok(read(relative).includes(expression), `${relative} must encode dynamic invoice ids`);
  }
});

test('keyboard focus and disabled actions remain visible', () => {
  const css = read('assets/css/app.css');
  for (const selector of ['.nav-link:focus-visible', '.btn:focus-visible', '.btn:disabled']) {
