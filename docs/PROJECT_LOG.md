...
- Status: `PLANNED` on `codex/release-integration-audit` after the product owner reported that GitHub changes were not appearing on Cloudflare production.
- Scope: inventory all GitHub/Jules branches and PRs, compare `main`, review, and Cloudflare artifacts by hash, audit CI/Wrangler/migrations/security changes, integrate the accepted invoice workstream onto current `main`, and publish a verified preview before any production merge.
- Initial evidence: Cloudflare production index hash exactly matches `origin/main`; the review preview index hash exactly matches `origin/codex/invoice-tax-print-options`. The deployment service is functioning, but production is stale because feature work remains unmerged.
- Safety boundary: do not merge the Jules “Final Release” PR or apply its migrations without review; its `unofficial` invoice enum conflicts with the active `ordinary` contract and duplicates fields already covered by applied migrations.
- Acceptance target: one integration branch based on current `main`, all required controls and migrations preserved, full tests and inline scripts passing, Cloudflare preview at the integration commit, browser smoke test, and an explicit merge decision with rollback evidence.
- Rollback: delete or close only the integration branch/PR; do not alter production data or the `main` ref until preview acceptance.
